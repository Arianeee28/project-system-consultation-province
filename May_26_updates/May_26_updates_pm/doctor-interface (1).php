...
function showSchedule() {
  ...
  container.appendChild(closeBtn);
    ...
    // Close calendar details container content
    function closeNames() {
      const container = document.getElementById('names-container');
      container.innerHTML = '';
    }

    // Change month and refresh calendar
    function changeMonth(offset) {
      ...
      showSchedule();
        ...
        // Patient tab function: Fetch and show appointments in patient-appointments-container for the selected day
    function showPatientAppointments() {
    const container = document.getElementById('patient-appointments-container');
    container.style.display = 'block';

      // Hide names container when showing patient appointments
    document.getElementById('names-container').innerHTML = '';  // clear out the calendar
  ...
  fetch(`get_appointments_doc.php?date=${formattedDate}`)
    ...
    .then(data => {
      if (data.length === 0) {
        container.innerHTML = `<h4>Appointments for ${months[selectedMonthIndex]} ${selectedDay}, ${year}:</h4><p>No appointments scheduled.</p>`;
      } else {
        let html = `<h4>Appointments for ${months[selectedMonthIndex]} ${selectedDay}, ${year}:</h4>`;
      ...
      closeBtn.innerHTML = 'X';
      closeBtn.classList.add('close-btn');
      closeBtn.onclick = closePatientAppointments;
      container.appendChild(closeBtn);
    })
    .catch(error => {
...
function closePatientAppointments() {
  const container = document.getElementById('patient-appointments-container');
  container.innerHTML = '';
